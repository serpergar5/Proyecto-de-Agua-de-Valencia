...
def menu1():
    ...
    while True:
        ...
        if i == 3:
            datos_usuarios = { "Fuente hídrica": que_identificador, "Calidad": calidad, "Litros": litros }
            añadir_mas_datos = str(
                input("¿Quieres darme los datos de otra fuente hídrica o salir al menu principal?: ")
            )
...
def menu2():
    ...
    while True:
        if i == 0:
            id_potabilizadora = str(input("Introduce un id para la planta potabilizadora: "))
            if not id_potabilizadora.isalnum() or not len(id_potabilizadora) >=3 or id_potabilizadora in plantas:
                print("Por favor, introduce un identificador valido")
            else:
                i += 1
                
        if i == 1:
            eficiencia = input("Ingresa la eficiencia de la planta potabilizadora " + str(eficiencia_lista)).title() + ": "
            if eficiencia not in eficiencia_lista:
                print(
                    "Por favor, introduce una eficiencia valida " + str(eficiencia_lista)).title() + ": "
            else:
                i += 1

        if i == 2:
            cantidad = int(input(
                "Cantidad de litros que potabiliza la planta potabilizadora: "))
            if calidad not in calidad_del_agua:
                print("Introduce una calidad válida.")
            else:
                i += 1
